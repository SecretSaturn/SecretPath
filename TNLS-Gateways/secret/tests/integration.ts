...
const initializeContract = async (
  ...
  const contractCodeHash = await client.query.compute.codeHashByCodeId({
    code_id: codeId,
  });
  console.log(`Contract hash: ${contractCodeHash}`);
...
async function initializeAndUploadContract() {
  ...
  const gatewayKey = Buffer.from(
    gatewayKeys.verification_key.substring(2),
    "hex",
  ).toString("base64");
...
async function queryPubKey(
  ...
  console.log(
    `\x1b[34;1mEth Address: ${computeAddress(response.verification_key)}\x1b[0m`,
  );
...
async function test_gateway_tx(
  ...
  console.log(`Sending query: {"get_public_keys": {} }`);
  const gatewayPublicKey = await queryPubKey(
    client,
    gatewayHash,
    gatewayAddress,
  );
